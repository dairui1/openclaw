...
export async function applyAuthChoiceApiProviders(
  ...
  if (
    ...
    } else if (params.opts.tokenProvider === "zenmux") {
  ...
  if (authChoice === "zenmux-api-key") {
    ...
    if (!hasCredential && params.opts?.token && params.opts?.tokenProvider === "zenmux") {
      await setZenmuxApiKey(normalizeApiKeyInput(params.opts.token), params.agentDir);
      hasCredential = true;
    }

    if (!hasCredential) {
      const envKey = resolveEnvApiKey("zenmux");
      if (envKey) {
        const useExisting = await params.prompter.confirm({
          message: `Use existing ZENMUX_API_KEY (${envKey.source}, ${formatApiKeyPreview(envKey.apiKey)})?`,
          initialValue: true,
        });
        if (useExisting) {
          await setZenmuxApiKey(envKey.apiKey, params.agentDir);
          hasCredential = true;
        }
      }
    }
    ...
    if (!hasCredential) {
      const key = await params.prompter.text({
        message: "Enter ZenMux API key",
        validate: validateApiKeyInput,
      });
      await setZenmuxApiKey(normalizeApiKeyInput(String(key)), params.agentDir);
      hasCredential = true;
    }
